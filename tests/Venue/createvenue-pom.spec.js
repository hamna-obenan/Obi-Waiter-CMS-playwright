...
test.describe('Venue Creation - Environment: ' + config.app.environment, () => {
  ...
  test("Complete venue creation flow with environment integration", async ({ page }) => {
    ...
    // Verify login success by checking we are redirected to the correct venue URL
    await expect(page).toHaveURL(config.urls.venue || /\/venue$/);
    ...
    // fill up the time zone
    await venuePage.fillTimeZone();
    console.log(`Time zone filled`);

    // fill up the currency
    await venuePage.fillCurrency();
    console.log(`Currency filled`);

    // fill up the cuisine
    await venuePage.fillCuisine();
    console.log(`Cuisine filled`);
    await expect(page.getByRole('textbox', { name: 'Cuisine' })).toHaveValue(config.testData.venue.cuisine);
